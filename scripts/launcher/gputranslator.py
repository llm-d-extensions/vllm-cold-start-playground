...
# NOTE (cold-start lab): the upstream module imports `kubernetes` at top level,
# but it is only used by the ConfigMap-based *mock* GPU path. The vLLM image
# does not ship the `kubernetes` package, and the real-GPU path (pynvml) never
# needs it, so the import is deferred into _load_gpu_map_from_configmap below.
# This does not change any measured launch behaviour.
